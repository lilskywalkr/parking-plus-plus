...
// Define the CalendarDate interface locally or import it
...
// 2. Define the Emitted Events for updates
const emit = defineEmits<{
    (e: 'update:calendarDate', value: CalendarDate | null): void;
    (e: 'update:formattedCalendarDate', value: string): void;
}>();
    ...
    <Form method="get" :action="route('statistics.show')" v-slot="{ errors, processing }" class="w-full flex justify-center">
        <DatePickerRoot :is-date-unavailable="(date) => date.day === 19" class="flex gap-4">
            ...
            <Input
                name="date"
                type="hidden"
                :model-value="formattedCalendarDate"
                @update:model-value="emit('update:formattedCalendarDate', $event)"
            />
            ...
            >
                ...
                <DatePickerCalendar v-slot="{ weekDays, grid }" class="p-4" :model-value="calendarDate" @update:model-value="$emit('update:calendarDate', $event)">
